// pages/EnhancedAnalyticsPage.tsx
...
const EnhancedAnalyticsPage: React.FC = () => {
  ...
  const handleRefresh = async () => { await refreshAllData() }
  ...
  const addToRecentlyViewed = (type: 'user' | 'project', id: number, name: string) => {
    ...
    if (type === 'user') updated.users = [newItem, ...updated.users.filter(u => u.id !== id)].slice(0, 5)
    else updated.projects = [newItem, ...updated.projects.filter(p => p.id !== id)].slice(0, 5)
  ...
  const unreadAlerts = performanceAlerts.filter(a => !a.is_read).length
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <div className="flex-1 space-y-6 p-4 md:p-6 max-w-full">
        ...
        {/* Quick Access / Actions (same grid rhythm as similar pages) */}
        ...
        {/* Date Range / Filters (carded, consistent spacing) */}
        <Card className="bg-card border-border">
          <CardHeader>
            <CardTitle className="text-foreground flex items-center gap-2">
        ...
        {/* Real-time stats (same grid cadence as other stats sections) */}
        {realTimeStats && (
              ...
              <Card className="bg-card border-border">
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-2 mb-4">
                    <Button variant="outline" onClick={() => handleTrendDataFetch('task_ratings')}>
                      Task Ratings
                    </Button>
                    <Button variant="outline" onClick={() => handleTrendDataFetch('tickets_solved')}>
                      Tickets Solved
                    </Button>
                    <Button variant="outline" onClick={() => handleTrendDataFetch('help_requests')}>
                      Help Requests
                    </Button>
                  </div>
                  {trendData.length > 0 && (
